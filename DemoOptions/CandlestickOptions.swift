    ...
    static func candlestickShOption() -> Option {
        ...
        let splitData: ([[Jsonable]]) -> [String: [Jsonable]] = { rowData in
            ...
            for data in rowData {
                var tmp = data
                tmp.removeFirst()
                categoryData.append(Float(tmp[0] as? Double ?? tmp[0] as! Int))
                values.append(tmp.map { Float($0 as? Double ?? $0 as! Int) })
            }
            return [
