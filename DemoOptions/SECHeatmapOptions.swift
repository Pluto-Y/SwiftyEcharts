//
//  SECHeatmapOptions.swift
//  SwiftyEcharts
//
//  Created by Pluto-Y on 16/01/2017.
//  Copyright © 2017 com.pluto-y. All rights reserved.
//

import SwiftyEcharts

public struct SECHeatmapOptions {
    
    // MARK: 热力图与百度地图扩展
    /// 地址: http://echarts.baidu.com/demo.html#heatmap-bmap
    static func heatmapBmapOption() -> SECOption {
        // TODO: 添加实现
        return SECOption(
        )
    }
    
    // MARK: 笛卡尔坐标系上的热力图
    /// 地址: http://echarts.baidu.com/demo.html#heatmap-cartesian
    static func heatmapCartesianOption() -> SECOption {
        // TODO: 添加实现
        return SECOption(
        )
    }
    
    // MARK: 热力图 - 颜色的离散映射
    /// 地址: http://echarts.baidu.com/demo.html#heatmap-large-piecewise
    static func heatmapLargePiecewiseOption() -> SECOption {
        // TODO: 添加实现
        return SECOption(
        )
    }
    
    // MARK: 热力图 - 2w 数据
    /// 地址: http://echarts.baidu.com/demo.html#heatmap-large
    static func heatmapLargeOption() -> SECOption {
        // TODO: 添加实现
        return SECOption(
        )
    }
    
    // MARK: 全国主要城市空气质量
    /// 地址: http://echarts.baidu.com/demo.html#heatmap-map
    static func heatmapMapOption() -> SECOption {
        // TODO: 添加实现
        return SECOption(
        )
    }

}
