...
class GraphsController: BaseDemoController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        menus = ["Les Miserables(暂缺)", "力引导布局(暂缺)", "力引导布局", "笛卡尔坐标系上的 Graph(暂缺)", "Graph Life Expectancy(暂缺)", "NPM Dependencies(暂缺)", "Graph 简单示例", "Graph Webkit Dep(暂缺)", "Les Miserables(暂缺)"]
